 ...
 * two neighbours contribute the lines y = 0 and x = 5000, which cross at
 * exactly the corner's own position — hence a landing of [5000, 0] to the
 * millimetre rather than merely close.
 */
...
test.describe('dragging a room corner', () => {
  ...
  test('the deadzone: 3px moves nothing, 5px arms the drag', async ({ app }) => {
    ...
    /* DEADZONE_PX is 4 canvas px from the press, so this is the one gesture
    ...
    /* 5px clears it. Arming does NOT replay the movement it swallowed — the
       corner goes to wherever the pointer is now. */
  ...
  test('two lines in reach: the corner lands on their crossing and reads "Right angle"', async ({ app }) => {
    ...
    /* Guide 1 hangs off corner 0 and runs along y = 0 to the landing point;
       guide 2 hangs off corner 2, [5000,4000], and runs up x = 5000. */
